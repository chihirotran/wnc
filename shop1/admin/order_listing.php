...
if (!empty($_SESSION['current_user'])) {
    ...
    if(!empty($where)){
        ...
        $totalRecords = mysqli_query($con, "SELECT * FROM `order` WHERE 1");
    }
    $totalRecords = $totalRecords->num_rows;
    $totalPages = ceil($totalRecords / $item_per_page);
    if(!empty($where)){
        $orders = mysqli_query($con, "SELECT * FROM `order` where (".$where.") ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }else{
        $orders = mysqli_query($con, "SELECT * FROM `order` ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }
    if (!$orders) {
        printf("Error: %s\n", mysqli_error($con));
        exit();
        ...
        <div class="listing-items">
            ...
            <ul>
                <li class="listing-item-heading">
                    ...
                    <div class="listing-prop listing-phone1">Trạng Thái</div>
                ...
                <?php  while ($row = mysqli_fetch_array($orders,MYSQLI_ASSOC)) { ?>
                <li>
                    ...
                    <div class="listing-prop listing-phone1"><?=$row['TrangThai']?></div>
                    <!-- <div class="listing-prop listing-button">
